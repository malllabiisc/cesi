...
class RESCAL(Model):
	...
	def __init__(self, *args, **kwargs):
		...
		aff = kwargs.pop('af', 'linear')				# Activation function to use (default=linear)
		self.add_hyperparam('af', af.afuns[aff])			# Add activation function hyperparam
		#self.add_param('E', (self.sz[0], self.ncomp), post=normless1)
		self.add_param('E', (self.sz[0], self.ncomp))			# Initialize embeddings for entities
		# TODO: support eigenvector initialization
		self.add_param('W', (self.sz[2], self.ncomp, self.ncomp))	# Intiailize embeddings for relations

	def scores(self, ss, ps, os):
		return np.array([						# Computes a_i . (R_k . a_j) for triple (i,j,k) | R_k is a 2-d matrix
			dot(self.E[ss[i]], dot(self.W[ps[i]], self.E[os[i]])) for i in range(len(ss))
		])
