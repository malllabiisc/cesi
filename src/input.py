...
class Input(object):
	def __init__(self, triples_list, amb_mentions, amb_ent, isAcronym):
		...
		self.kbpLinking()
		self.amieInfo()
		self.tokenOverlap(amb_mentions, amb_ent)
	...
	def fixTypos(self, amb_ent, amb_mentions, isAcronym):
		...
		for trp in self.triples:
			sub_n, rel_n, obj_n 	= trp['triple_n']
			sub, rel, obj 		= trp['triple']
			sub = sub.split('|')[0]
		...
		ent2ppdb = getPPDBclustersRaw(sub_list)
		...
		for rep, clust in rep2ent.items():
			config.sideInfo_fw.write(rep + '\n')
			for ele in clust:
				config.sideInfo_fw.write('\t' + ele + '\n')
	...
	def fixEasy(self):
		...
		for ele in self.triples:
			sub, _, _ = ele['triple']
			sub = sub.split('|')[0]

			for tok in sub.split():
				uni_clust[tok].add(sub)

			for t1, t2 in itertools.combinations(sub.split(), 2):
				bi_clust[(t1, t2)].add(sub)

		uf = DisjointSet()
		for rep, clust in uni_clust.items():
			if len(clust) == 2:
				clust = list(clust)
				uf.add(clust[0], clust[1])

			if len(clust) > 2 and rep in clust: amb_ent[rep] = 1


		for bi, clust in bi_clust.items():
			if len(clust) == 2:
				clust = list(clust)
				uf.add(clust[0], clust[1])
	...
	def wikiLinking(self):
		for trp in self.triples:
			...
			if trp['wiki_sub_lnk'] != None:
				self.ent2wiki[sub_id] = self.ent2wiki.get(sub_id, set())
				self.ent2wiki[sub_id].update(trp['wiki_sub_lnk'])

			if trp['wiki_obj_lnk'] != None:
				self.ent2wiki[obj_id] = self.ent2wiki.get(obj_id, set())
				self.ent2wiki[obj_id].update(trp['wiki_obj_lnk'])
	...
	def tokenOverlap(self, amb_mentions, amb_ent):
		...
		for ent in self.ent_list:
			...
			for tok in val.split():
				if tok in config.stpwords: continue
				cluster[tok].add(ent)
		...
		for rep, clust in cluster.items():
			for e1, e2 in itertools.combinations(clust, 2):
				...
				tokens1 = set(e1.split()) - set(config.stpwords);
				tokens2 = set(e2.split()) - set(config.stpwords);
		...
		for rel in self.rel_list:
			for tok in rel.split(): 
				self.rel_term_freq[tok] = self.rel_term_freq[tok]+1 if tok not in config.stpwords else self.rel_term_freq[tok]
		...
		for rep, clust in cluster.items():
			for r1, r2 in itertools.combinations(clust, 2):
				...
				tokens1 = set(r1.split()) - set(config.stpwords);
				tokens2 = set(r2.split()) - set(config.stpwords);
	...
	def printCluster(self, id2rep, id2name, ctype = 'm2o'):
		...
		for rep, cluster in inv_dict.items():
			...
			config.sideInfo_fw.write(str(rep) + '\n')
			for ele in cluster: 
				config.sideInfo_fw.write('\t' + ele + '\n')
